...
public class RawBinaryGltfDataReaderV2 {
    ...
    public static RawGltfData readBinaryGltf(ByteBuffer data) throws IOException {
        ...
        if (jsonChunk.type != CHUNK_TYPE_JSON) {
            throw new IOException("First chunk must be of type JSON (" + CHUNK_TYPE_JSON
                    + "), but found " + jsonChunk.type);
        }
        ...
        if (chunks.size() > 1) {
            ...
            if (binChunk.type != CHUNK_TYPE_BIN) {
                throw new IOException("Second chunk must be of type BIN (" + CHUNK_TYPE_BIN
                        + "), but found " + jsonChunk.type);
            }
    ...
    private static List<Chunk> readChunks(ByteBuffer data) throws IOException {
        ...
        while (offset < data.capacity()) {
            ...
            if (offset + chunk.length > data.capacity()) {
                throw new IOException("The offset for the data of chunk " + chunks.size() + " is "
                        + offset + ", its length is " + chunk.length + ", but "
                        + (offset + chunk.length)
