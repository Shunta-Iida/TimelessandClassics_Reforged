...
public final class GunRenderType extends RenderType {
    private static final RenderType BULLET_TRAIL = RenderType.create(
            Reference.MOD_ID + ":projectile_trail", DefaultVertexFormat.POSITION_COLOR_LIGHTMAP,
    ...
    // private static final RenderType SCREEN = RenderType.create(Reference.MOD_ID +
    // ":screen_texture", DefaultVertexFormat.NEW_ENTITY, VertexFormat.Mode.QUADS,
    // 1024, true, false,
    ...
    private static final RenderTypeCreator RENDER_TYPE_CREATOR = new RenderTypeCreator() {
        ...
        public RenderType create() {
            RenderType.CompositeState renderState = RenderType.CompositeState.builder()
                    ...
                    .setTextureState(new TextureStateShard(GunRenderingHandler.MUZZLE_FLASH_TEXTURE,
                            false, false))
    ...
    private static final RenderType MUZZLE_SMOKE = RenderType.create(
            Reference.MOD_ID + ":muzzle_smoke", DefaultVertexFormat.POSITION_COLOR_TEX,
            VertexFormat.Mode.QUADS, 256, true, false,
            RenderType.CompositeState.builder()
                    .setShaderState(RenderStateShard.POSITION_COLOR_TEX_SHADER)
                    .setTextureState(new RenderStateShard.TextureStateShard(
                            GunRenderingHandler.MUZZLE_SMOKE_TEXTURE, false, false))
                    .setTransparencyState(TRANSLUCENT_TRANSPARENCY).setCullState(NO_CULL)
                    .createCompositeState(true));
    private static final RenderType SCREEN_BLACK =
            RenderType.create(Reference.MOD_ID + ":screen_black", DefaultVertexFormat.NEW_ENTITY,
                    VertexFormat.Mode.QUADS, 64, true, false,
                    RenderType.CompositeState.builder()
                            .setTextureState(new RenderStateShard.TextureStateShard(
                                    GunRenderingHandler.MUZZLE_SMOKE_TEXTURE, false, false))
                            .createCompositeState(false)); // 256

    private GunRenderType(String nameIn, VertexFormat formatIn, VertexFormat.Mode drawModeIn,
            int bufferSizeIn, boolean useDelegateIn, boolean needsSortingIn, Runnable setupTaskIn,
            Runnable clearTaskIn) {
        super(nameIn, formatIn, drawModeIn, bufferSizeIn, useDelegateIn, needsSortingIn,
                setupTaskIn, clearTaskIn);
    }
