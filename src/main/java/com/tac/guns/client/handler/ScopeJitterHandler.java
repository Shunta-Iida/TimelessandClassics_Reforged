    ...
    public void tick() {
        if (ticksBeforeBreathing > 0) {
            ...
            if (!isStabilizing() || forceBreathing) {
                ...
                if (radix % (long) (duration * component / 2) <= 2)
                    xTarget = Math.random() * 2 - 1;
                if (respiratoryQuotient > 0)
                    respiratoryQuotient -= 5;
                if (respiratoryQuotient <= 0)
                    forceBreathing = false;
            } else {
                respiratoryQuotient++;
                if (respiratoryQuotient >= suffocationPoint * component)
                    forceBreathing = true;
            }
        }
    }
