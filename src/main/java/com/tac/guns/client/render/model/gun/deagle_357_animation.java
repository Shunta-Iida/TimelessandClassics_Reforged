    ...
    public void render(GunSkin skin, float partialTicks, ItemTransforms.TransformType transformType,
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    Deagle50AnimationController.INDEX_MAG, transformType, matrices);
        ...
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    Deagle50AnimationController.EXTRA_MAG, transformType, matrices);
            if (controller.isAnimationRunning())
                if (controller
                        .isAnimationRunning(GunAnimationController.AnimationLabel.RELOAD_EMPTY))
                    isEmp = true;
                else if (!controller
                        .isAnimationRunning(GunAnimationController.AnimationLabel.STATIC))
                    isEmp = false;
            if (transformType.firstPerson() && (!isEmp
                    || (controller.isAnimationRunning(GunAnimationController.AnimationLabel.STATIC)
                            && !isEmp)))
                renderMag(stack, matrices, renderBuffer, light, overlay, skin);
        }
        matrices.popPose();
        ...
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    Deagle50AnimationController.INDEX_BODY, transformType, matrices);
        ...
        controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                Deagle50AnimationController.INDEX_SLIDE, transformType, matrices);
