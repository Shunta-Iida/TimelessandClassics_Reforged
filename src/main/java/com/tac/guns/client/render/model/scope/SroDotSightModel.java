...
public class SroDotSightModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType,
        ...
        if (!parent.isEmpty()) {
            ...
            if (gunItem.getGun().getModules().getAttachments().getPistolScope()
                    .getDoRenderMount()) {
                RenderUtil.renderModel(MINI_DOT_BASE.getModel(), stack, matrixStack,
        ...
        if (transformType.firstPerson() && entity.equals(Minecraft.getInstance().player)) {
            matrixStack.pushPose();
            {
                ...
                // ScopeEditor.get().getScopeData().getTagName() != "sro_dot" ? new
                // ScopeData("") : ScopeEditor.get().getScopeData();
                // matrixStack.translate(scopeData.getReticleXMod(), scopeData.getReticleYMod(),
                // 0);
                ...
                matrixStack.mulPose(
                        Vector3f.YP.rotationDegrees(GunRenderingHandler.get().newSwayYaw * 0.85f));
                matrixStack.mulPose(Vector3f.ZN
                        .rotationDegrees(GunRenderingHandler.get().newSwayPitch * 0.85f));
                matrixStack
