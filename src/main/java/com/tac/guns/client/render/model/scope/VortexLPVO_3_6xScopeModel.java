...
public class VortexLPVO_3_6xScopeModel implements IOverrideModel {
    private static final ResourceLocation RED_DOT_RETICLE = new ResourceLocation(Reference.MOD_ID,
            "textures/items/timeless_scopes/razor_lpvo_reticle.png");
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType,
        ...
        if (transformType.firstPerson() && entity.equals(Minecraft.getInstance().player)) {
            ScopeData scopeData = ScopeEditor.get().getScopeData() == null
                    || ScopeEditor.get().getScopeData().getTagName() != "vlpvo6" ? new ScopeData("")
            ...
            matrixStack.pushPose();
            {
                ...
                matrixStack.translate((-size / 2) + scopeData.getDrXZoomMod(),
                        (0.0685075 + 0.01175 - 0.01225) + scopeData.getDrYZoomMod(),
                        (Config.CLIENT.display.scopeDoubleRender.get()
                                ? (4.65 + 0.52975 - 0.618 + scopeData.getDrZZoomMod()) * 0.0625
                                : (2.15 + 0.52975 - 0.618 + scopeData.getDrZZoomMod()) * 0.0625));
                ...
                if (Config.CLIENT.display.scopeDoubleRender.get()) {
                     ...
                     * FlipTextures flipTextures = new FlipTextures(Shaders.activeProgram.getName(),
                     * Shaders.activeProgram.getDrawBuffers().capacity());
                     * 
                     * GL43.glCopyImageSubData(3553, GL11.GL_TEXTURE_2D, 0, 0, 0, 0, MIRROR_TEX,
                     * GL11.GL_TEXTURE_2D, 0, 0, 0, 0,
                     * ScreenTextureState.instance().lastWidth,
                     * ScreenTextureState.instance().lastHeight,
                     * 1);
                     * } else {
                     * GL43.glCopyTexImage2D(GL_TEXTURE_2D, 0,
                     * GL_RGB,0,0,ScreenTextureState.instance().lastWidth,
                     * ScreenTextureState.instance().lastHeight,0);
                     * }
                     */
                    ...
                    /*
                     * builder = renderTypeBuffer.getBuffer(GunRenderType.getScreen());
                     * builder.pos(matrix, 0, size, 0).color(color, color, color, 1.0F).tex(texU,
                     * 1.0F - crop).overlay(overlay).lightmap(15728880).normal(normal, 0.0F, 1.0F,
                     * 0.0F).endVertex();
                     * builder.pos(matrix, 0, 0, 0).color(color, color, color, 1.0F).tex(texU,
                     * crop).overlay(overlay).lightmap(15728880).normal(normal, 0.0F, 1.0F,
                     * 0.0F).endVertex();
                     * builder.pos(matrix, size, 0, 0).color(color, color, color, 1.0F).tex(1.0F -
                     * texU, crop).overlay(overlay).lightmap(15728880).normal(normal, 0.0F, 1.0F,
                     * 0.0F).endVertex();
                     * builder.pos(matrix, size, size, 0).color(color, color, color, 1.0F).tex(1.0F
                     * - texU, 1.0F - crop).overlay(overlay).lightmap(15728880).normal(normal, 0.0F,
                     * 1.0F, 0.0F).endVertex();
                     */
                }
