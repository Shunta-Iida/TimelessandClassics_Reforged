...
public class colt_python_animation extends SkinnedGunModel {
    ...
    public void render(GunSkin skin, float partialTicks, ItemTransforms.TransformType transformType,
        ...
        controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                COLTPYTHONAnimationController.INDEX_BODY, transformType, matrices);
        RenderUtil.renderModel(getComponentModel(skin, BODY), stack, matrices, renderBuffer, light,
                overlay);
        RenderUtil.renderModel(getComponentModel(skin, SIGHT_LIGHT), stack, matrices, renderBuffer,
                15728880, overlay);
        matrices.popPose();

        matrices.pushPose();
        controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                COLTPYTHONAnimationController.INDEX_MAG, transformType, matrices);
        RenderUtil.renderModel(getComponentModel(skin, MAG), stack, matrices, renderBuffer, light,
                overlay);
        matrices.popPose();
        ...
        controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                COLTPYTHONAnimationController.INDEX_BODY, transformType, matrices);
        // if
        // ((!(controller.getAnimationFromLabel(GunAnimationController.AnimationLabel.RELOAD_NORMAL).equals(controller.getPreviousAnimation())
        // &&
        ...
        if (!SyncedEntityData.instance().get(player, ModSyncedDataKeys.RELOADING)
            ...
            if (cooldownOg < 0.74) {
                ...
                matrices.translate(0, -(cooldownOg * 1.74F) * 0.135,
                        0.0625F * (cooldownOg * 1.74F));
            }
        }
        RenderUtil.renderModel(getComponentModel(skin, TacGunComponents.HAMMER), stack, matrices,
        ...
        if (cooldownOg < 0.74) {
            ...
            matrices.translate(1.45 * (cooldownOg * 1.74F) * 0.0625,
                    -0.625 * (cooldownOg * 1.74F) * 0.0625, 0);
        }
        RenderUtil.renderModel(getComponentModel(skin, TacGunComponents.ROTATE), stack, matrices,
                renderBuffer, light, overlay);
        matrices.popPose();
        ...
        if ((controller.getAnimationFromLabel(GunAnimationController.AnimationLabel.RELOAD_NORMAL)
            ...
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    COLTPYTHONAnimationController.INDEX_LOADER, transformType, matrices);
            RenderUtil.renderModel(getComponentModel(skin, TacGunComponents.LOADER), stack,
        ...
        matrices.pushPose();
        controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                COLTPYTHONAnimationController.INDEX_BULLET1, transformType, matrices);
        if (cooldownOg < 0.74) {
            matrices.mulPose(Vector3f.ZN.rotationDegrees(-45F * (cooldownOg * 1.74F)));
            matrices.translate(1.45 * (cooldownOg * 1.74F) * 0.0625,
                    -0.625 * (cooldownOg * 1.74F) * 0.0625, 0);
        ...
        if ((controller.getAnimationFromLabel(GunAnimationController.AnimationLabel.RELOAD_NORMAL)
                ...
                && transformType.firstPerson() && controller.isAnimationRunning()) {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    COLTPYTHONAnimationController.INDEX_BULLET2, transformType, matrices);
