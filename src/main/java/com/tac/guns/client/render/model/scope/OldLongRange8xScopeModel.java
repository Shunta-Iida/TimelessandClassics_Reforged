...
public class OldLongRange8xScopeModel implements IOverrideModel {
    private static final ResourceLocation RED_DOT_RETICLE = new ResourceLocation(Reference.MOD_ID,
            "textures/items/timeless_scopes/standard_old_8x_scope_reticle.png");
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType,
         ...
         * //double zScale = AimingHandler.get().getNormalisedRepairProgress() >= 0.125
         * ? 0.375D + 0.95D * (1.0D - transition) : 0.05D + 0.95D * (1.0D - transition);
        ...
        if (transformType.firstPerson() && entity.equals(Minecraft.getInstance().player)) {
            ScopeData scopeData = ScopeEditor.get().getScopeData() == null
                    || ScopeEditor.get().getScopeData().getTagName() != "old8x" ? new ScopeData("")
            ...
            matrixStack.pushPose();
            {
                ...
                matrixStack.translate((-size / 2) + scopeData.getDrXZoomMod(),
                        (0.0888 - 0.00355) + scopeData.getDrYZoomMod(),
                        ((4.445 + scopeData.getDrZZoomMod()) - 0.54) * 0.0625);// 4.815 * 0.0625);
                ...
                matrixStack.translate((-0.00455715) + scopeData.getReticleXMod(),
                        (-0.004415) + scopeData.getReticleYMod(),
                ...
                double invertZoomProgress = aimed ? 0.0575 : 0.468;// double invertZoomProgress = aimed ? 0.135 :
                                                                   ...
                                                                   // : ;
                                                                   // matrixStack.translate(-0.095*Math.asin(((double)
                ...
                // -GunRenderingHandler.get().kickReduction)*0.15);
                matrixStack.translate(0, 0, -0.35);
                matrixStack
                        .mulPose(Vector3f.YN.rotationDegrees((GunRenderingHandler.get().newSwayYaw
                                * GunRenderingHandler.get().recoilReduction) * 0.075f));
                matrixStack.mulPose(Vector3f.ZN.rotationDegrees((GunRenderingHandler
                        ...
                        * GunRenderingHandler.get().recoilReduction) * 0.075f)); // seems to be interesting to
                ...
                matrixStack
                        .mulPose(Vector3f.XP.rotationDegrees((GunRenderingHandler.get().recoilLift
                                * GunRenderingHandler.get().recoilReduction) * 0.0625F));
                matrixStack.translate(0, 0, 0.35);
            ...
            matrixStack.popPose();
        } // float scopeSize = 1.095F;matrixStack.translate(-size / 2, 0.0645 , 3.45 *
          // 0.0625);
    }
}
