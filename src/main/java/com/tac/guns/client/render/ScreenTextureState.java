...
public class ScreenTextureState extends RenderStateShard.TexturingStateShard {
    ...
    public static ScreenTextureState instance() {
        return ScreenTextureState.instance == null
                ? ScreenTextureState.instance = new ScreenTextureState()
                : ScreenTextureState.instance;
    }
    ...
    private ScreenTextureState() {
        super("screen_texture", () -> {
            ...
            RenderSystem.setShaderTexture(0, ScreenTextureState.instance().getTextureId());
        }, () -> {
    ...
    private void onRenderWorldLast(final RenderLevelStageEvent event) {
        if (event.getStage().equals(RenderLevelStageEvent.Stage.AFTER_WEATHER)) {
            final Window mainWindow = Minecraft.getInstance().getWindow();

            // OpenGL will spit out an error (GL_INVALID_VALUE) if the window is minimised
            // (or draw calls stop)
            // It seems just testing the width or height if it's zero is enough to prevent
            // it
            if (mainWindow.getScreenWidth() <= 0 || mainWindow.getScreenHeight() <= 0)
                return;

            RenderSystem.bindTexture(this.getTextureId());
            if (mainWindow.getScreenWidth() != this.lastWindowWidth
                    || mainWindow.getScreenHeight() != this.lastWindowHeight) {
                // When window resizes the texture needs to be re-initialized and copied, so
                // both are done in the same call
                ...
                GL11.glCopyTexImage2D(GL11.GL_TEXTURE_2D, 0, GL11.GL_RGB, 0, 0,
                        mainWindow.getWidth(), mainWindow.getHeight(), 0);
            } else {
                // Copy sub-image is faster than copy because the texture does not need to be
                // initialized
                GL11.glCopyTexSubImage2D(GL11.GL_TEXTURE_2D, 0, 0, 0, 0, 0, mainWindow.getWidth(),
 ...
 *//*
              ...
              * mc.getWindow().getScreenWidth(), mc.getWindow().getScreenHeight(), 0,
              ...
              * //glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, 9728); // This final
 ...
 *//*
              ...
              * 
              * 
              * // Next im guessing i'll have to check this world render, it says the render
              * is incomplete, maybe this code is supposed to complete it for the specific
              * image output?
              * 
              ...
              * if(this.textureId==-1||(lastWidth!=mc.getWindow().getScreenWidth()||
 ...
 * return (50.0f / ((ModelAttachment)
 * itemAttachment.type.model).config.sight.fovZoom);
 * }
 *//*
              * 
...
/* RenderSystem *//*
                   ...
                   * 
                   * //GL30.glBindFramebuffer(mc.getFramebuffer().getColorTextureId(),
                   * this.textureId);
                   * //GL30.glBindTexture(mc.getFramebuffer().getColorTextureId(),
                   * this.textureId);
                   * int tex = mc.getMainRenderTarget().getColorTextureId();
                   * //TACOptifineShadersHelper.setFramebufferTexture(mc.getMainRenderTarget(),
                   * this.textureId);// = MIRROR_TEX;
                   * //GL30.glFramebufferTexture2D(GL30.GL_FRAMEBUFFER, GL30.GL_COLOR_ATTACHMENT0,
                   * GL11.GL_TEXTURE_2D, this.textureId, 0);
                   * if(mc.levelRenderer != null && mc.gameRenderer != null) {
                   * mc.gameRenderer.render(partialTick, endTime, true);
                   * //mc.gameRenderer.renderWorld(partialTick, endTime, new
                   * MatrixStack());//gameRenderer.renderWorld(partialTick, endTime + (1000000000
                   * / 30), matrixStackWrl);
                   * }
                   * else
                   ...
                   * //GL30.glCopyTexSubImage2D(Minecraft.getInstance().getFramebuffer().
                   * getColorTextureId(), GL11.GL_TEXTURE_2D, 0, 0, 0, 0, MIRROR_TEX,
                   * GL11.GL_TEXTURE_2D, 0, 0, 0, 0, lastWidth, lastHeight, 1);
                   * 
                   ...
                   * Gun.getScope(stack) != null &&
                   * Gun.getScope(stack).getAdditionalZoom().getDrCropZoom() > 0) {
                   * //RenderSystem.pushMatrix();
                   * //TODO: Figure out why this is never called, the copy doesn't even work when
                   * I force it, do I care getting this working?
                   * GL43.glDeleteTextures(ScreenTextureState.instance().textureId);
                   * GL43.glDeleteBuffers(ScreenTextureState.instance().textureId);
 ...
 *//*
             ...
             */
/*
 * GL43.glCopyImageSubData(Shaders.activeProgram.getDrawBuffersCustom().get(0)
 * TACOptifineShadersHelper.getFlipTextures().getA(0), 3553, 0,0,0, 0,
 * this.textureId, 3553,0,0,0,0, mc.getWindow().getScreenWidth(),
 ...
 *//*
              ...
              * } else {
              * //RenderSystem.bindTexture(this.textureId);
              * GL43.glCopyTexImage2D(GL_TEXTURE_2D, 0,
              * GL_RGB,0,0,mc.getWindow().getScreenWidth(),
              ...
              * this.lastWindowWidth = mainWindow.getScreenWidth();
              * this.lastWindowHeight = mainWindow.getScreenHeight();
              * GL43.glCopyTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, 0, 0, mainWindow.getWidth(),
              * mainWindow.getHeight(), 0);
              * }
              * else
              * {
              * // Copy sub-image is faster than copy because the texture does not need to be
              * initialized
              * GL43.glCopyTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, 0, 0, mainWindow.getWidth(),
              * mainWindow.getHeight());
              * }
              * }
              * }
