    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType,
        ...
        if (transformType.firstPerson() && entity.equals(Minecraft.getInstance().player)) {
            ScopeData scopeData = ScopeEditor.get().getScopeData() == null
                    || ScopeEditor.get().getScopeData().getTagName() != "srsdot" ? new ScopeData("")
            ...
            matrixStack.pushPose();
            {
                ...
                matrixStack.translate(((-size / 2) - 0.0027 + scopeData.getReticleXMod()),
                        (0.50 + 0.29223 + scopeData.getReticleYMod()) * 0.0625,
                        (0.3 + 0.0875 + 0.4255 + scopeData.getReticleZMod()) * 0.0625);
                ...
                matrixStack.mulPose(
                        Vector3f.YP.rotationDegrees(GunRenderingHandler.get().newSwayYaw * 0.8f));
                matrixStack.mulPose(
                        Vector3f.ZN.rotationDegrees(GunRenderingHandler.get().newSwayPitch * 0.8f));
                matrixStack
