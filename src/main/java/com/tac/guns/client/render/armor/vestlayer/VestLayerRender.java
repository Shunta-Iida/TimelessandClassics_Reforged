...
/**
 * Author: Inspiration from MrCrayfish backpack code, my biggest problem was
 * understanding RenderTypes and why I should learn more about them. This file
 * is still a test however, all vest items must still
 * have a way to change their textures, along with each vest being a separate
 * render setup rather then render-er.
 *
 * TODO: Enable for eventual bot system, this is for Biped, not just player
 * models...
 */
@OnlyIn(Dist.CLIENT)
    ...
    public void render(PoseStack stack, MultiBufferSource renderTypeBuffer, int p_225628_3_,
        ...
        if (!WearableHelper.PlayerWornRig(player).isEmpty()
            ...
            stack.pushPose();
            {
                ...
                model.rotateToPlayerBody(this.getParentModel().body); // Default rotation, keep for now? It's a global,
                                                                      // maybe force more work on the model
