    ...
    public void render(GunSkin skin, float partialTicks, ItemTransforms.TransformType transformType,
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    MRADAnimationController.INDEX_BODY, transformType, matrices);
            ...
            RenderUtil.renderModel(getComponentModel(skin, TacGunComponents.BARREL), stack,
                    matrices, renderBuffer, light, overlay);
            matrices.translate(0, 0, 0.3);
            RenderUtil.renderModel(getComponentModel(skin, TacGunComponents.BIPOD), stack, matrices,
        ...
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    MRADAnimationController.INDEX_HANDLE, transformType, matrices);
            RenderUtil.renderModel(getComponentModel(skin, BOLT_TAIL), stack, matrices,
                    renderBuffer, light, overlay);
        }
        matrices.popPose();
        ...
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    MRADAnimationController.INDEX_BOLT, transformType, matrices);
        ...
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY),
                    MRADAnimationController.INDEX_MAGAZINE, transformType, matrices);
        ...
        matrices.pushPose();
        {
            if (controller.isAnimationRunning()) {
                ...
                if (controller
                        .getAnimationFromLabel(GunAnimationController.AnimationLabel.PULL_BOLT)
                        .equals(controller.getPreviousAnimation())) {
                    RenderUtil.renderModel(getComponentModel(skin, BULLET_SHELL), stack, matrices,
                            renderBuffer, light, overlay);
                } else {
