...
public class MicroHoloSightModel implements IOverrideModel {
    ...
    public void render(float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack,
         ...
         * AimingHandler.get().getNormalisedAdsProgress(), 2.0D);
        ...
        if (transformType.firstPerson() && entity.equals(Minecraft.getInstance().player)) {
            matrixStack.pushPose();
            {
                 ...
                 * newSwayYaw * GunRenderingHandler.get().recoilReduction)*0.75F));
                 * matrixStack.rotate(Vector3f.ZP.rotationDegrees((GunRenderingHandler.get().
                 * newSwayYaw * GunRenderingHandler.get().weaponsHorizontalAngle * 0.65f *
                 * GunRenderingHandler.get().recoilReduction)*0.75F)); // seems to be
                 * interesting to
