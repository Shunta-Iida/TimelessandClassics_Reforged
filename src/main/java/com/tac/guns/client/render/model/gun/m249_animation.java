...
public class m249_animation extends SkinnedGunModel {
    ...
    public void render(GunSkin skin, float partialTicks, ItemTransforms.TransformType transformType, ItemStack stack,
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY), M249AnimationController.INDEX_BODY,
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY), M249AnimationController.INDEX_MAGAZINE,
                    transformType, matrices);
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY), M249AnimationController.INDEX_CAPS,
                    transformType, matrices);
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY), M249AnimationController.INDEX_HANDLE,
                    transformType, matrices);
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY), M249AnimationController.INDEX_CHAIN,
                    transformType, matrices);
            if (controller.isAnimationRunning(GunAnimationController.AnimationLabel.RELOAD_EMPTY)
                    || Gun.hasAmmo(stack)) {
                RenderUtil.renderModel(getComponentModel(skin, BULLET_CHAIN), stack, matrices, renderBuffer, light,
                        overlay);
            }
        }
        matrices.popPose();
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY), M249AnimationController.INDEX_ROTATE,
                    transformType, matrices);
        ...
        matrices.pushPose();
        {
            controller.applySpecialModelTransform(getComponentModel(skin, BODY), M249AnimationController.INDEX_IRON,
                    transformType, matrices);
            RenderUtil.renderModel(getComponentModel(skin, TacGunComponents.BULLET_CHAIN_COVER), stack, matrices,
