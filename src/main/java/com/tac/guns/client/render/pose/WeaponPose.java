...
public abstract class WeaponPose implements IHeldAnimation {
    ...
    protected float getPlayerPitch(final Player player) {
        ...
        return Mth.lerp(Minecraft.getInstance().getFrameTime(), player.xRotO, player.getXRot())
                / 90F;
    }
    ...
    private void applyAimPose(final AimPose targetPose, final ModelPart rightArm,
        ...
        this.applyLimbPoseToModelRenderer(targetPose.getIdle().getRightArm(),
                ...
                sneaking);
        this.applyLimbPoseToModelRenderer(targetPose.getIdle().getLeftArm(),
                targetPose.getAiming().getLeftArm(), this.forwardPose.getIdle().getLeftArm(),
                this.forwardPose.getAiming().getLeftArm(), leftArm, partial, zoom, offhand,
                sneaking);
    }
    ...
    private void applyLimbPoseToModelRenderer(final LimbPose targetIdlePose,
        ...
        renderer.yRot = (float) Math.toRadians(this.getValue(targetIdlePose.getRotationAngleY(),
                targetAimingPose.getRotationAngleY(), idlePose.getRotationAngleY(),
                aimingPose.getRotationAngleY(), renderer.yRot, partial, zoom, leftHanded));
        renderer.zRot = (float) Math.toRadians(this.getValue(targetIdlePose.getRotationAngleZ(),
                targetAimingPose.getRotationAngleZ(), idlePose.getRotationAngleZ(),
                aimingPose.getRotationAngleZ(), renderer.zRot, partial, zoom, leftHanded));
        renderer.x = this.getValue(targetIdlePose.getRotationPointX(),
                targetAimingPose.getRotationPointX(), idlePose.getRotationPointX(),
                aimingPose.getRotationPointX(), renderer.x, partial, zoom, leftHanded);
        renderer.y = this.getValue(targetIdlePose.getRotationPointY(),
                targetAimingPose.getRotationPointY(), idlePose.getRotationPointY(),
                aimingPose.getRotationPointY(), renderer.y, partial, zoom, 1F)
                + (sneaking ? 2F : 0F);
        renderer.z = this.getValue(targetIdlePose.getRotationPointZ(),
                targetAimingPose.getRotationPointZ(), idlePose.getRotationPointZ(),
