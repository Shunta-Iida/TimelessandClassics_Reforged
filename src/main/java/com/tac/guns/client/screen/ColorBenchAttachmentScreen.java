...
public class ColorBenchAttachmentScreen extends Screen {
     ...
     * super.init();
     * 
     * this.btnApply = this.addButton(new Button(this.guiLeft + 195, this.guiTop +
     * 16, 74, 20, new TranslatableComponent("gui.tac.workbench.assemble"), button
     * ->
     * {
     * PacketHandler.getPlayChannel().sendToServer(new MessageCraft(registryName,
     * this.workbench.getPos()));
     * }));
     ...
     * if(item.getItem() instanceof IColored && ((IColored)
     * item.getItem()).canColor(item))
     * {
     ...
     * colored.setColor(item, ((red & 0xFF) << 16) | ((green & 0xFF) << 8) | ((blue
     * & 0xFF)));
     * }
     * else
     * {
     ...
     * 
     * if(RenderUtil.isMouseWithin((int) mouseX, (int) mouseY, startX + 26, startY +
     * 17, 142, 70))
     * {
     * if(!this.mouseGrabbed && (button == GLFW.GLFW_MOUSE_BUTTON_LEFT || button ==
