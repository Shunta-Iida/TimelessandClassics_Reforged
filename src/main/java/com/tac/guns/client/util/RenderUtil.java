...
public class RenderUtil {
    ...
    public static void renderModel(ItemStack stack, PoseStack matrixStack, MultiBufferSource buffer, int light,
            int overlay, @Nullable LivingEntity entity) {
        renderModel(stack, ItemTransforms.TransformType.NONE, matrixStack, buffer, light, overlay, entity);
    }

    public static void renderModel(ItemStack child, ItemStack parent, PoseStack matrixStack, MultiBufferSource buffer,
    ...
    public static void renderModel(ItemStack stack, ItemTransforms.TransformType transformType, PoseStack matrixStack,
            MultiBufferSource buffer, int light, int overlay, @Nullable LivingEntity entity) {
    ...
    public static void renderModel(ItemStack stack, ItemTransforms.TransformType transformType, PoseStack matrixStack,
            MultiBufferSource buffer, int light, int overlay, @Nullable Level world, @Nullable LivingEntity entity) {
        BakedModel model = Minecraft.getInstance().getItemRenderer().getModel(stack, world, entity,
                entity == null ? 0 : entity.getId());
        renderModel(model, transformType, stack, matrixStack, buffer, light, overlay);
    }

    public static void renderModel(BakedModel model, ItemStack stack, PoseStack matrixStack, MultiBufferSource buffer,
            int light, int overlay) {
        renderModel(model, ItemTransforms.TransformType.NONE, stack, matrixStack, buffer, light, overlay);
    }

    public static void renderLaserModuleModel(BakedModel model, ItemStack stack, PoseStack matrixStack,
            MultiBufferSource buffer, int light, int overlay) {
        renderLaserModuleModel(model, ItemTransforms.TransformType.NONE, stack, matrixStack, buffer, light, overlay);
    }

    public static void renderLaserModuleModel(BakedModel model, ItemTransforms.TransformType transformType,
            ItemStack stack, PoseStack matrixStack, MultiBufferSource buffer, int light, int overlay) {
        renderModel(model, transformType, null, stack, ItemStack.EMPTY, matrixStack, buffer, light, overlay, true);
    }

    public static void renderModel(BakedModel model, ItemTransforms.TransformType transformType, ItemStack stack,
    ...
    public static void renderModel(BakedModel model, ItemTransforms.TransformType transformType,
        ...
        if (!stack.isEmpty()) {
            ...
            model = net.minecraftforge.client.ForgeHooksClient.handleCameraTransforms(matrixStack, model, transformType,
                    false);
            ...
            if (!model.isCustomRenderer() && (stack.getItem() != Items.TRIDENT || flag)) {
                ...
                if (model.isLayered()) {
                    net.minecraftforge.client.ForgeHooksClient.drawItemLayered(
                            Minecraft.getInstance().getItemRenderer(), model, stack, matrixStack, buffer, light,
                            overlay, flag1);
                } else {
                    ...
                    if (renderWithPersonalColor) {
                        renderModelPersonallyColored(model, stack, parent, transform, matrixStack, builder, light,
                                overlay);
                    } else
                        renderModel(model, stack, parent, transform, matrixStack, builder, light, overlay);
                }
            } else {
                RenderProperties.get(stack.getItem()).getItemStackRenderer().renderByItem(stack, transformType,
     ...
     * @param overlay
     */
    private static void renderModelPersonallyColored(BakedModel model, ItemStack stack, ItemStack parent,
            @Nullable Transform transform, PoseStack matrixStack, VertexConsumer buffer, int light, int overlay) {
        if (transform != null) {
            transform.apply();
        }
        ...
        for (Direction direction : Direction.values()) {
            random.setSeed(42L);
            renderPersonalizedQuads(matrixStack, buffer, model.getQuads(null, direction, random), stack, parent, light,
                    overlay);
        }
        random.setSeed(42L);
     ...
     * @param overlay
     */
    private static void renderModel(BakedModel model, ItemStack stack, ItemStack parent, @Nullable Transform transform,
            PoseStack matrixStack, VertexConsumer buffer, int light, int overlay) {
        if (transform != null) {
            transform.apply();
        }
        Random random = new Random();
     ...
     * @param overlay
     */
    private static void renderPersonalizedQuads(PoseStack matrixStack, VertexConsumer buffer, List<BakedQuad> quads,
            ItemStack stack, ItemStack parent, int light, int overlay) {
        PoseStack.Pose entry = matrixStack.last();
     ...
     * @param overlay
     */
    private static void renderQuads(PoseStack matrixStack, VertexConsumer buffer, List<BakedQuad> quads,
            ItemStack stack, ItemStack parent, int light, int overlay) {
        PoseStack.Pose entry = matrixStack.last();
        for (BakedQuad quad : quads) {
            float alpha = 1f;
    ...
    public static int getItemStackColor(ItemStack stack, ItemStack parent, int tintIndex) {
        ...
        if (stack != null && !Gun.getAttachment(IAttachment.Type.SCOPE_BODY_COLOR, stack).isEmpty()) {
            color = ((DyeItem) Gun.getAttachment(IAttachment.Type.SCOPE_BODY_COLOR, stack).getItem()).getDyeColor()
                    .getTextColor();
        }
        return color;
    }

    public static int getItemStackColor(ItemStack stack, ItemStack parent, IAttachment.Type attachmentType,
            int tintIndex) {
    ...
    public static void applyTransformType(ItemStack stack, PoseStack matrixStack,
        ...
        BakedModel model = Minecraft.getInstance().getItemRenderer().getModel(stack, entity.level, entity,
                entity.getId());
    ...
    public static void renderFirstPersonArm(LocalPlayer player, HumanoidArm hand, PoseStack matrixStack,
