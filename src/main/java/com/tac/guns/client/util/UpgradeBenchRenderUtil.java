...
public class UpgradeBenchRenderUtil implements BlockEntityRenderer<UpgradeBenchTileEntity> {
     ...
     *                        rendered at - used to interpolate frames between
     *                        ticks, to make animations smoother. For example - if
     *                        the frame rate is steady at 80 frames per second,
     *                        this method will be called four times per tick, with
     *                        partialTicks spaced 0.25 apart, (eg) 0, 0.25, 0.5,
     *                        0.75
     * @param matrixStack     the matrixStack is used to track the current view
     *                        transformations that have been applied - i.e
     *                        translation, rotation, scaling
     *                        it is needed for you to render the view properly.
     * @param renderBuffers   the buffer that you should render your model to
     * @param combinedLight   the blocklight + skylight value for the tileEntity.
     *                        see
     *                        http://greyminecraftcoder.blogspot.com/2014/12/lighting-18.html
     *                        (outdated, but the concepts are still valid)
     * @param combinedOverlay value for the "combined overlay" which changes the
     *                        render based on an overlay texture (see OverlayTexture
     *                        class).
     *                        Used by vanilla for (1) red tint when a living entity
     *                        takes damage, and (2) "flash" effect for creeper when
     *                        ignited
    ...
    public void render(UpgradeBenchTileEntity tileEntityMBE21, float partialTicks,
        ...
        /*
         * if(Config.COMMON.development.enableTDev.get() && (ObjectRenderEditor.get() !=
         * null && ObjectRenderEditor.get().currElement == 1 &&
         * ObjectRenderEditor.get().GetFromElements(1) != null)) {
         * matrixStack.translate(ObjectRenderEditor.get().GetFromElements(1).getxMod(),
         * ObjectRenderEditor.get().GetFromElements(1).getyMod(),
         * ObjectRenderEditor.get().GetFromElements(1).getzMod());
         * }
         */
        matrixStack.translate(-0.14, -0.4200001, 0);
