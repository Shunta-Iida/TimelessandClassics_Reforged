...
public class ChildHeadshotBox<T extends LivingEntity> extends BasicHeadshotBox<T> {
    ...
    public ChildHeadshotBox(double headWidth, double headHeight, double headYOffset, double childHeadScale,
            double headYOffsetScale) {
    ...
    public AABB getHeadshotBox(T entity) {
        ...
        if (headBox != null && entity.isBaby()) {
            return new AABB(headBox.minX * this.childHeadScale, headBox.minY * this.headYOffsetScale,
                    headBox.minZ * this.childHeadScale, headBox.maxX * this.childHeadScale,
                    headBox.maxY * (this.headYOffsetScale + 0.065), headBox.maxZ * this.childHeadScale);
        }
        return headBox;
    }
}
