...
/**
 * An attachment class related to scopes. Scopes need to at least specify the
 * additional zoom (or fov)
 * they provide and the y-offset to the center of the scope for them to render
 * correctly. Use
 * {@link #create(ScopeZoomData[], double, double, String, IGunModifier...)} to
 * create an get.
 * <p>
...
public class Scope extends Attachment {
    ...
    /**
     * Marks this scope to allow it to be stabilised while using a controller. This
     * is essentially
    ...
    /**
     * Sets the offset distance from the camera to the view finder when Double
     * Render is enabled
     *
    ...
    public ScopeZoomData getAdditionalZoom() {
        if (this.zoomData == null || AimingHandler.get() == null)
            return new ScopeZoomData(1f, 0); // Null, loader might attempt to hit scope data when aimed is detected
                                             // before init
        if (this.zoomData.length <= AimingHandler.get().getCurrentScopeZoomIndex()) {
            AimingHandler.get().resetCurrentScopeZoomIndex();
            return this.zoomData[AimingHandler.get().getCurrentScopeZoomIndex()];// new ScopeZoomData(0, 0); // Skip
                                                                                 // Null
        }
    ...
    /**
     * Gets the offset to the center of the scope. Used to render scope cross hair
     * exactly in the
     ...
     * @param centerOffset   the length to the center of the view finder from the
     *                       base of the scope model in pixels
