...
public class TimelessGunItem extends GunItem {
        ...
        public TimelessGunItem(Process<Item.Properties> properties, Boolean integratedOptic,
        ...
        public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip,
                        ...
                        if (tagCompound.getBoolean("IgnoreAmmo")) {
                                ...
                                tooltip.add((new TranslatableComponent("info.tac.ammo",
                                                ChatFormatting.GOLD.toString() + ammoCount + "/"
                                                                + GunModifierHelper.getAmmoCapacity(stack,
                                                                                modifiedGun)))
                                                .withStyle(ChatFormatting.DARK_GRAY));
                        }
                }
                ...
                if (!isShift) {
                        ...
                        tooltip.add(
                                        (new TranslatableComponent("info.tac.more_info_gunitem",
                                                        new KeybindComponent(Keys.MORE_INFO_HOLD.getKey().toString())))
                ...
                if (isShift) {
                        ...
                        if (tagCompound != null) {
                                double armorPen = gun.getGun().getProjectile().getGunArmorIgnore() >= 0
                                                ? Math.min((Config.COMMON.gameplay.percentDamageIgnoresStandardArmor
                                                                .get()
                                                                * gun.getGun().getProjectile().getGunArmorIgnore()
                                                                * 100), 100F)
                                                : 0F;
                                ...
                                int headDamgeModifier = Config.COMMON.gameplay.headShotDamageMultiplier.get()
                                                * gun.getGun().getProjectile().getGunHeadDamage() >= 0
                                                                ? (int) (Config.COMMON.gameplay.headShotDamageMultiplier
                                                                                .get()
                                                                                * gun.getGun().getProjectile()
                                                                                                .getGunHeadDamage()
                                                                                * 100)
                                                                : 0;
                        ...
                        tooltip.add((new TranslatableComponent("info.tac.attachment_help",
                                        Keys.ATTACHMENTS.getKey().getDisplayName().getContents()
        ...
        public boolean isBarVisible(ItemStack stack) {
                if (Config.CLIENT.display.weaponAmmoBar.get()) {
                        ...
                        return !tagCompound.getBoolean("IgnoreAmmo")
                                        && tagCompound.getInt("AmmoCount") != GunModifierHelper.getAmmoCapacity(stack,
                                                        modifiedGun);
                } else
                        return false;
        }
