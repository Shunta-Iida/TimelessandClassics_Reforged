...
public class TimelessGunItem extends GunItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (ammo != null) {
            tooltip.add((new TranslatableComponent("info.tac.ammo_type",
                    new TranslatableComponent(ammo.getDescriptionId()).withStyle(ChatFormatting.GOLD))
        ...
        tooltip.add((new TranslatableComponent("info.tac.damage", ChatFormatting.GOLD
                + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format(additionalDamage) + additionalDamageText))
                .withStyle(ChatFormatting.DARK_GRAY));
        if (tagCompound != null) {
        ...
        if (!isShift) {
            ...
            tooltip.add(
                    (new TranslatableComponent("info.tac.more_info_gunitem", new KeybindComponent(Keys.MORE_INFO_HOLD.getKey().toString()))).withStyle(ChatFormatting.YELLOW));
        }
        if (isShift) {
            GunItem gun = (GunItem) stack.getItem();
            if (tagCompound != null) {
                double armorPen = gun.getGun().getProjectile().getGunArmorIgnore() >= 0
                        ? Math.min((Config.COMMON.gameplay.percentDamageIgnoresStandardArmor.get()
                                * gun.getGun().getProjectile().getGunArmorIgnore() * 100), 100F)
                        : 0F;
                tooltip.add((new TranslatableComponent("info.tac.armorPen",
                        new TranslatableComponent(String.format("%.1f", armorPen) + "%").withStyle(ChatFormatting.RED))
                        .withStyle(ChatFormatting.DARK_AQUA)));

                int headDamgeModifier = Config.COMMON.gameplay.headShotDamageMultiplier.get()
                ...
                float percentageToNextLevel = (tagCompound.getFloat("levelDmg") * 100)
                        / (modifiedGun.getGeneral().getLevelReq() * (((tagCompound.getInt("level")) * 3.0f)));
                tooltip.add((new TranslatableComponent("info.tac.current_level").append(new TranslatableComponent(" "
                        + tagCompound.getInt("level") + " : " + String.format("%.2f", percentageToNextLevel) + "%")))
            ...
            tooltip.add((new TranslatableComponent("info.tac.attachment_help",
                    Keys.ATTACHMENTS.getKey().getDisplayName().getContents().toUpperCase(Locale.ENGLISH)))
            ...
            if (gun.getGun().canAttachType(IAttachment.Type.PISTOL_SCOPE))
                tooltip.add((new TranslatableComponent("info.tac.pistolScope",
                        new TranslatableComponent("MiniScope").withStyle(ChatFormatting.BOLD))
                        .withStyle(ChatFormatting.LIGHT_PURPLE)));
            if (gun.getGun().canAttachType(IAttachment.Type.IR_DEVICE))
                tooltip.add((new TranslatableComponent("info.tac.irLaserEquip",
                        new TranslatableComponent("IrLaser").withStyle(ChatFormatting.BOLD))
    ...
    public boolean isBarVisible(ItemStack stack) {
        if (Config.CLIENT.display.weaponAmmoBar.get()) {
            ...
            Gun modifiedGun = this.getModifiedGun(stack);
            return !tagCompound.getBoolean("IgnoreAmmo")
                    && tagCompound.getInt("AmmoCount") != GunModifierHelper.getAmmoCapacity(stack, modifiedGun);
        } else
            return false;
    }
