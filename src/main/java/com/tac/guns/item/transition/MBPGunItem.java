...
public class MBPGunItem extends TimelessGunItem {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level worldIn, List<Component> tooltip, TooltipFlag flag) {
        ...
        if (tagCompound != null && tagCompound.contains("AdditionalDamage", 99)) {
            ...
            if (additionalDamage > 0.0F) {
                additionalDamageText = ChatFormatting.GREEN + " +"
                        + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format((double) additionalDamage);
        ...
        tooltip.add((new TranslatableComponent("info.tac.damage", new Object[] { ChatFormatting.GOLD
                + ItemStack.ATTRIBUTE_MODIFIER_FORMAT.format((double) additionalDamage) + additionalDamageText }))
                .withStyle(ChatFormatting.DARK_GRAY));
        if (tagCompound != null) {
            if (tagCompound.getBoolean("IgnoreAmmo")) {
                ...
                tooltip.add((new TranslatableComponent("info.tac.ammo",
                        new Object[] { ChatFormatting.GOLD.toString() + ammoCount + "/"
        ...
        if (tagCompound != null) {
            if (tagCompound.get("CurrentFireMode") == null) {
                ...
                tooltip.add((new TranslatableComponent("info.tac.firemode_safe",
                        new Object[] {
                                (new KeybindComponent("key.tac.fireSelect")).getString().toUpperCase(Locale.ENGLISH) }))
                        .withStyle(ChatFormatting.GREEN));
            else if (tagCompound.getInt("CurrentFireMode") == 1)
                tooltip.add((new TranslatableComponent("info.tac.firemode_semi",
                        new Object[] {
                                (new KeybindComponent("key.tac.fireSelect")).getString().toUpperCase(Locale.ENGLISH) }))
                        .withStyle(ChatFormatting.RED));
            else if (tagCompound.getInt("CurrentFireMode") == 2)
                tooltip.add((new TranslatableComponent("info.tac.firemode_auto",
                        new Object[] {
                                (new KeybindComponent("key.tac.fireSelect")).getString().toUpperCase(Locale.ENGLISH) }))
                        .withStyle(ChatFormatting.RED));
        }
        ...
        if (tagCompound != null) {
            ...
            float speed = 0.1f / (1 + ((gun.getGun().getGeneral().getWeightKilo()
                    * (1 + GunModifierHelper.getModifierOfWeaponWeight(stack))
                    + GunModifierHelper.getAdditionalWeaponWeight(stack)) * 0.0275f));
            speed = Math.max(Math.min(speed, 0.095F), 0.075F);
            if (speed > 0.094f)
                tooltip.add((new TranslatableComponent("info.tac.lightWeightGun",
                        new TranslatableComponent(-((int) ((0.1 - speed) * 1000)) + "%").withStyle(ChatFormatting.RED))
            ...
            // tooltip.add((new TranslatableComponent("info.tac.oldRifleScope", new
            // TranslatableComponent("OldScope").withStyle(ChatFormatting.BOLD)).withStyle(ChatFormatting.LIGHT_PURPLE)));
        }
        ...
        if (tagCompound != null) {
            ...
            tooltip.add((new TranslatableComponent("info.moonshine.oldRifleScope",
                    new TranslatableComponent("Rain").withStyle(ChatFormatting.BOLD)).withStyle(ChatFormatting.BLUE)));
        }
        tooltip.add((new TranslatableComponent("info.tac.attachment_help",
                new Object[] { (new KeybindComponent("key.tac.attachments")).getString().toUpperCase(Locale.ENGLISH) }))
