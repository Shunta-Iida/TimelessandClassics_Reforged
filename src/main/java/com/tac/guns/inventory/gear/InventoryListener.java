...
public class InventoryListener {
     ...
     * public static void onEntityJoin(EntityJoinWorldEvent event) throws
     * InvocationTargetException, IllegalAccessException {
     * if(!(event.getEntity() instanceof PlayerEntity)) return;
     * 
     * PlayerEntity player = (PlayerEntity) event.getEntity();
     * if(addSlotMethod == null) {
     * addSlotMethod = ObfuscationReflectionHelper.findMethod(Container.class,
     * "addSlot", Slot.class);
     * }
     * // GearSlotsHandler wearableItemHandler = (GearSlotsHandler)
     * player.getCapability(ITEM_HANDLER_CAPABILITY).resolve().get();
     * // addSlotMethod.invoke(player.container, new
     * ArmorRigSlot(wearableItemHandler, 0, 170, 84)); // Rig
     * // addSlotMethod.invoke(player.container, new
     * BackpackSlot(wearableItemHandler, 1, 170, 102)); // Backpack
     * }
     */

    /*
     * @SubscribeEvent
     * public static void onAttachCapabilities(AttachCapabilitiesEvent<Entity>
     * event) throws InvocationTargetException, IllegalAccessException {
    ...
    @SubscribeEvent
    public static void onAttachCapabilitiesStack(AttachCapabilitiesEvent<ItemStack> event)
